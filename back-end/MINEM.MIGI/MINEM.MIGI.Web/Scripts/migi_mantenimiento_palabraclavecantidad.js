...
var renderizar = (data, cantidadCeldas, pagina, registros) => {
    ...
    if (deboRenderizar) {
        contenido = data.map((x, i) => {
            ...
            let btnCambiarEstado = `${[0, 1].includes(x.FLAG_ESTADO) ? "" : `<a class="dropdown-item btnCambiarEstado" href="javascript:void(0)" data-id="${x.ID_PALABRA_CANTIDAD}" data-estado="${x.FLAG_ESTADO}"><i class="fas fa-eraser mr-1"></i>Eliminar</a>`}`;
            let btnEditar = `<a class="dropdown-item btnEditar" href="#" data-id="${x.ID_PALABRA_CANTIDAD}" data-toggle="modal" data-target="#modal-mantenimiento"><i class="fas fa-edit mr-1"></i>Editar</a>`;
...
var consultarDatos = (element) => {
    ...
    $('#exampleModalLabel').html('Actualización de palabra clave cantidad <br><small class="text-muted">Puede cambiar los datos mostrados para actualizar una palabra clave cantidad</small><small class="text-danger d-block"><strong>(*)&nbsp;</strong>Campos obligatorios</small>');
...
var nuevo = () => {
    ...
    $('#btnGuardar').show();
    $('#btnGuardar').next().html('Cancelar');
    $('#exampleModalLabel').html('Registro de nueva palabra clave cantidad <br><small class="text-muted">Complete los siguientes campos para registrar una nueva palabra clave cantidad</small><small class="text-danger d-block"><strong>(*)&nbsp;</strong>Campos obligatorios</small>');
}
